...
class Links_model extends CI_Model {
  ...
  */
    ...
    function get_siteconstants(  ) {
        ...
        $this->db->from($this->config->item('pb_links'));
    ...
    function get_all_entries()
    {
        ...
        $query = $this->db->get($this->config->item('pb_links'));
        return $query->result();
    }
    //-------------------------------------------------------------------------------------------
    /*
    ...
    */
    ...
    function get_links()
    {
        ...
        $result=$this->db->order_by('linkdescription','asc')->from($this->config->item('pb_links'))->get();
        ...
        if( $result->num_rows() > 0 )
        {
            foreach($result->result_array() as $row)
            {
                $dropdown[$row['id']]=$row['linkdescription'] .', '.'';
    ...
    function insert_entry($data)    //
    {
        ...
        $this->db->insert('pb_links', $data);		
        */
        $this->linkdescription = $data['linkdescription'];
        $this->linktype = $data['linktype'];
        $this->targeturl = $data['targeturl'];
        $this->db->insert($this->config->item('pb_links'), $this);
    ...
    function update_entry($data)
    {
		...
		*/
        ...
        $this->db->update($this->config->item('pb_links'), $data);
    ...
    {
        $this->db->delete($this->config->item('pb_links'), array('id' => $id));
    ...
    function get_entry($id){
        $this->db->where('id', $id);
        $query = $this->db->get($this->config->item('pb_links'), 1);
        return $query->result();
    }
    //-------------------------------------------------------------------------------------------
...
/* Location: ./application/models/admin/links_model.php */